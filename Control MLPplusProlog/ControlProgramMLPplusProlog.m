...
% CreateMatlabSim@gmail.com
    ...
    x=org.jpl7.Query('consult(''/home/igormacedo/Documents/MATLAB/iRobotCreateSimulatorToolbox/Control MLPplusProlog/prologControl.pl'')');
    x.hasSolution;
    ...
    while toc(tStart) < maxDuration
        ...
        %datasonar = [distSonar(1); distSonar(2); distSonar(4)];
        
        if min(datalidar) >= 0.4
        
            [r , ~] = mlp.activate(datalidar);
            rightWheel = r(1,end);
            leftWheel = r(2,end);
            
        else
            
            c = strcat('movimento(', num2str(datalidar(5)),',', num2str(datalidar(4)),',',num2str(datalidar(3)),',',num2str(datalidar(2)),',',num2str(datalidar(1)),',','ME,MD).');
            %c = 'movimento(SEE,SE,SF,SD,SDD,ME,MD).';
            x=org.jpl7.Query(c);
            r = x.allSolutions;
