...
% CreateMatlabSim@gmail.com
    ...
    while toc(tStart) < maxDuration
        ...
        %serPort.travelDist(0.5,1);
        
        objective = ag_run([pos(1) pos(2)],datasonar,serPort.thAbs);

        tg = (objective(2) - pos(2))/(objective(1)-pos(1));
        
        orientation = serPort.thAbs
        
        if sqrt((4.5-pos(1))^2+(4.5-pos(2))^2) > 0.5
            plot(objective(1),objective(2),'r.-');
             
            if atan(tg) > 0 && (objective(2) - pos(2)) >= 0
                arctan = atan(tg)
                rot = serPort.thAbs - atan(tg)        
                serPort.turnAngle(0.4, -rot*180/pi);
                serPort.travelDist(0.5,0.4); 

            elseif atan(tg) < 0 && (objective(2) - pos(2)) >= 0
                arctan = atan(tg)
                rot = 3.14 + atan(tg) - serPort.thAbs
                serPort.turnAngle(0.4, rot*180/pi);
                serPort.travelDist(0.5,0.4); 

            elseif atan(tg) > 0 && (objective(2) - pos(2)) < 0
                arctan = atan(tg)
                rot = -3.14 - serPort.thAbs + atan(tg)        
                serPort.turnAngle(0.4, +rot*180/pi);
                serPort.travelDist(0.5,0.4); 
